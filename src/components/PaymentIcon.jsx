// Logos de medios de pago para el Footer. Mismo criterio que
// SocialIcon.jsx/WhatsAppIcon.jsx: SVG inline a mano en vez de archivos de
// imagen sueltos, salvo Naranja X (no hay versión vectorial fácil de
// conseguir con licencia clara, así que va como PNG en public/images/pagos/).
//
// Fuente y licencia de cada uno (ago 2026):
// - Visa, Mercado Pago: Simple Icons (simpleicons.org), licencia CC0 1.0 —
//   repositorio de logos de marca pensado justo para este uso (mostrar
//   "aceptamos tal medio de pago"). Colores oficiales ya incluidos en el
//   path (no usan currentColor a propósito).
// - Mastercard: el símbolo oficial vigente (círculos, sin texto, el que usa
//   la marca desde el 7 ene 2019) sacado directo de
//   brand.mastercard.com/brandcenter/mastercard-brand-mark/downloads.html
//   vía Wikimedia Commons (File:Mastercard_2019_logo.svg, "Mastercard
//   Incorporated" como autor) — la versión de Simple Icons lo reducía a un
//   solo color y perdía el rojo/naranja característico.
// - Naranja X: logo oficial (wordmark), de Wikimedia Commons, marcado como
//   dominio público (no supera el umbral de originalidad por ser solo
//   texto/formas simples) — ver commons.wikimedia.org/wiki/File:Naranja_X.png
export default function PaymentIcon({ name, className = 'h-6 w-auto' }) {
  if (name === 'visa') {
    return (
      <svg viewBox="0 0 24 24" className={className} fill="#1A1F71" role="img" aria-label="Visa">
  ...
  if (name === 'mastercard') {
    return (
      <svg viewBox="0 0 1000 618" className={className} role="img" aria-label="Mastercard">
  ...
  if (name === 'mercadopago') {
    return (
      <svg viewBox="0 0 24 24" className={className} fill="#00B1EA" role="img" aria-label="Mercado Pago">
  ...
  if (name === 'naranjax') {
    return <img src="/images/pagos/naranja-x.png" alt="Naranja X" className={className} />
  }
  return null
}
